...
int susfs_spoof_cmdline_or_bootconfig(struct seq_file *m)
{
	...
	do {
		...
		if (unlikely(!fake_cmdline_or_bootconfig))
			return 0;
...
void susfs_get_enabled_features(void __user **user_info) {
	...
	defined(CONFIG_KSU_SUSFS_SUS_KSTAT) || \
	defined(CONFIG_KSU_SUSFS_SPOOF_UNAME) || \
	defined(CONFIG_KSU_SUSFS_ENABLE_LOG) || \
	defined(CONFIG_KSU_SUSFS_HIDE_KSU_SUSFS_SYMBOLS) || \
	defined(CONFIG_KSU_SUSFS_SPOOF_CMDLINE_OR_BOOTCONFIG) || \
	defined(CONFIG_KSU_SUSFS_OPEN_REDIRECT) || \
	defined(CONFIG_KSU_SUSFS_SUS_MAP) || \
	defined(CONFIG_KSU_SUSFS_SUS_OVERLAYFS)
